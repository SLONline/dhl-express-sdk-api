...
class ShipmentRequestBuilder implements ShipmentRequestBuilderInterface
{
    ...
    public function addExportItem(
        ...
        $netWeightDetails   = $this->normalizeWeight($netWeight, strtoupper($netWeightUOM));
        $grossWeightDetails = $this->normalizeWeight($grossWeight, strtoupper($grossWeightUOM));
        
        $this->data['exportItems'][] = [
            'itemNumber'                => $itemNumber,
            'quantity'                  => $quantity,
            'quantityUnitOfMeasurement' => $quantityUnitOfMeasurement,
            'itemDescription'           => $itemDescription,
            'unitPrice'                 => $unitPrice,
            'netWeight'                 => round((float)$netWeightDetails['weight'], 3),
            'netWeightUOM'              => $netWeightDetails['uom'],
            'grossWeight'               => round((float)$grossWeightDetails['weight'], 3),
            'grossWeightUOM'            => $grossWeightDetails['uom'],
            'manufacturingCountryCode'  => $manufacturingCountryCode,
    ...
    public function build(): ShipmentRequestInterface
    {
        ...
        $exportItems = [];
        if (\array_key_exists('exportItems', $this->data) && \is_array($this->data['exportItems'])) {
            foreach ($this->data['exportItems'] as $item) {
                $exportItems[] = new ExportItem(
                    $item['itemNumber'],
                    $item['quantity'],
                    $item['quantityUnitOfMeasurement'],
                    ...
                    $item['grossWeight'],
                    $item['grossWeightUOM'],
                    $item['manufacturingCountryCode'],
                    $item['commodityCode'],
                    $item['ECCN'],
                    $item['exportReasonType'],
                    $item['taxesPaid']
                );
            }
